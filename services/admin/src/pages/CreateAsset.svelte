			...
			<button
				...
				{disabled}
					on:click={(e) => {
						...
						if (!assetTypeId) {
							errorMessage = "Choose an item type before creating the asset.";
							return;
						}
						const isPackage = assetTypeId === "32";
						const fileInput = document.getElementById("rbxm") as HTMLInputElement | null;
						if (!isPackage && (!fileInput || !fileInput.files || fileInput.files.length === 0)) {
							errorMessage = "Choose an .RBXM file before creating the asset.";
							return;
						}
					...
					if (fileInput && fileInput.files && fileInput.files[0]) {
						bodyFormData.append("rbxm", fileInput.files[0]);
					}
					if (assetTypeId === "32") {
