...
const MyDashboard = props => {
  ...
  useEffect(() => {
    ...
    getFriends({
      ...
      console.error('[error] dashboard friends request failed', e);
    });
  }, [auth.userId]);
    ...
    getGameSorts({
      ...
      Promise.all(proms).then(() => {
        ...
        multiGetUniverseIcons({
          ...
          console.error('[error] dashboard thumbnail request failed', e);
        })
      }).catch(e => {
        console.error('[error] dashboard game list request failed', e);
      })
    }).catch(e => {
      console.error('[error] dashboard game sorts request failed', e);
    })
  }, [auth.userId]);
