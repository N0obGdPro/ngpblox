...
#if DEBUG
// The default Windows Event Log provider can throw when the development
// process is not elevated. A logger failure must never close an otherwise
// valid HTTP response (for example, a 401 from a protected API endpoint).
...
#if DEBUG
// Do not use the Windows user-profile DPAPI key ring in local development.
// Those keys can become unreadable when the site runs under another account,
// preventing Razor from issuing antiforgery tokens on /auth pages.
// The ephemeral provider also avoids persisting an unencrypted dev key, whose
// warning cannot be logged on machines without Windows Event Log access.
builder.Services.AddDataProtection()
    .UseEphemeralDataProtectionProvider();

// Give each debug run a fresh cookie name. This makes browsers ignore an
// antiforgery cookie created by a previous run, whose ephemeral key no longer
// exists after the server restarts.
builder.Services.AddAntiforgery(options =>
{
    options.Cookie.Name = "RobloxDevAntiforgery-" + Guid.NewGuid().ToString("N");
});
#endif
...
app.UseEndpoints(e =>
{
    // The auth pages live in Razor rather than the proxied 2016 frontend.
    // Keep the bare route out of Next.js, then send it to the actual site home.
    e.MapGet("/auth", () => Results.Redirect("/home"));
