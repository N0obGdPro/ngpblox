        ...
        try
        {
            if (ctx.Request.Cookies.ContainsKey(CookieName))
            {
                ...
                if (!string.IsNullOrEmpty(decodedResult.sessionId))
                {
                    ...
                    // Application approval is required by the public site, but a
                    // local debug instance must allow test accounts to reach the
                    // frontend without entering the external application flow.
#if DEBUG
                    var appStatus = true;
#else
