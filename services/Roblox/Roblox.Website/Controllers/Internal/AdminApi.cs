...
-The Roblox Team"
    ...
    public async Task<dynamic> CopyAssetFromRoblox([Required, FromBody] CopyAssetRequest request)
    {
        ...
        Roblox.Libraries.RobloxApi.ProductDataResponse details;
        Stream content;
        try
        {
            details = await services.robloxApi.GetProductInfo(request.assetId, true);
            content = await services.robloxApi.GetAssetContent(request.assetId);
        }
        catch (HttpRequestException)
        {
            throw new StaffException("Could not reach Roblox's public asset API. Copying Roblox assets is unavailable on this machine right now; use Asset Create to upload a local file instead.");
        }
    ...
    public async Task<dynamic> CreateAsset([Required, FromForm] CreateAssetRequest request)
    {
        ...
        if (isPackage)
        {
            if (string.IsNullOrWhiteSpace(request.packageAssetIds))
                throw new StaffException("Must specify assetIds when creating a package");
            try
            {
                packageAssetIds = request.packageAssetIds.Split(",", StringSplitOptions.RemoveEmptyEntries)
                    .Select(id => long.Parse(id.Trim())).ToList();
            }
            catch (FormatException)
            {
                throw new StaffException("Package asset IDs must be a comma-separated list of numbers");
            }
            if (!packageAssetIds.Any())
                throw new StaffException("Must specify at least one assetId when creating a package");
