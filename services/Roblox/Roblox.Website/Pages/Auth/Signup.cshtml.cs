...
public class Signup : RobloxPageModel
{
    ...
    public async Task<IActionResult> OnPost()
    {
        ...
        // Every new creator starts with one editable baseplate. Keep the guard
        // so this never creates duplicates if the signup flow is retried.
        var existingGames = await services.games.GetGamesForType(
            CreatorType.User, createdUser.userId, 1, 0, "asc", "All");
        if (!existingGames.Any())
        {
            var starterPlace = await services.assets.CreatePlace(
                createdUser.userId, CreatorType.User, createdUser.userId);
            await services.games.CreateUniverse(starterPlace.placeId);
        }
